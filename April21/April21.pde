//Treedom Zhang
//Assignment due 21 April
//Special thanks to Brian!
//Original code from professor Sherwood
...
class Rotater {
    ...
    for (Rotater r : rotaters) {
      if (r.index != index) {
        if (dist(x, y, r.x, r.y)<lineDistance) {
          line(x, y, r.x, r.y);
          // println (r.speed);
          float weightbecauseofspeed;
          weightbecauseofspeed= map (r.speed, 0, 3, 0, 500);
          strokeWeight (weightbecauseofspeed);//  the weight of the stroke is set accoding to the speed of the rotater, becase the speed is so small so I make the difference obvious by mapping it and setting the range 0-500
        }
      }
    }
  }
  ...
  void checkMouse() {
    ...
    if (dist(mouseX, mouseY, x, y)>200 ) {// If your mouse moves to a position and stop it, the lines around and close to your mouse will slow down and remain static as if you capture it 
      acceleration=.002;
    }
  }
}
