//Final Project  : Candy & Bug
//Author         : Treedom Zhang
//Acknowledgement: Brian Kim for the inspiration on the mouseClicked function to change screen
//                 Professor Shiloh and Henry Yu for the instruction on the picking and setting of the colors
//                 The design of the elements in cheerUp array comes from Daniel Shiffman https://processing.org/examples/bouncingball.html
//Description    : In this game, you use your mouse to control the position of your bug and will score by eating the candies on the screen, before the start of the game you can pick a color 
//                 for your bug and your score will be shown on the end screen.
...
void colorChoosingScreen() {//in this part I only show the color without set the color in the bug display
...
void mouseClicked() {
  ...
  } else if (screenN == 3 ) { //if we click the mouse on color picking screen we will go into the game and also the color picked will be passed on to the bug display 
...
//Class Cheerup——————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————————
class Cheerup {      // The design of the elements in cheerUp array comes from Daniel Shiffman https://processing.org/examples/bouncingball.html
...
class Bar {
  ...
  void bugEat () { 
    ...
    if (x+5<=mouseX+myBug.radiusHead && x>=mouseX-myBug.radiusHead && y>=mouseY-myBug.radiusHead && y+5<=mouseY+myBug.radiusHead) { // if the bar enters the head
      ...
      x+=width;
     // display=false;// in order to let the candy disappear
    }

    if ( x > width*3) {// this comes from multiple trials to see when is the best time to end game screen into end screen
      smooth();
      for (int i = 0; i< myCheerups.length; i++) {
...
class Bug {
  ...
  float radiusHead = 20;
  float radiusBody = 14;
  color chosencolor= color(17, 173, 43);
  Bug () {
    xpos = width/2;
    ypos = height/2;
    xspeed = 1;
